...
int main()
{
  ...
  for (auto &default_ITER : {50})
  {
    ...
    for (std::string matrixName : matrixNames)
    {
      ...
      for (auto &method : methods)
      {
        ...
        for (int seed = 1; seed <= 10; seed++)
        {
          ...
          if (seed == 1)
            problem.printOutput(positions, "doc/main/overall/out/" + problem.matrixName + "_" +
                                               MethodStr[method] + "_" +
    ...
    std::string fileName = "doc/main/overall/hist_" + std::to_string(default_ITER) + ".txt";
  ...
  // Visualize a worst case
  {
    ...
    assert(!hist.empty());
    problem.printOutput(
        positions, "doc/main/overall/out/" + problem.matrixName + "_" + MethodStr[method] +
                       "_200.out");
  }
