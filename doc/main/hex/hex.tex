  ...
  \foreach \X in {0,1,2}{
      \begin{scope}[xshift=3*\X cm]
        ...
        \foreach \x in {0,1,2}{
            \foreach \y in {0,1}{
                \ifthenelse{
                  \(\X=1 \AND \x=1 \AND \y=1\) \OR
                  \(\X=1 \AND \x=0 \AND \y=1\) \OR
                  \(\X=1 \AND \x=1 \AND \y=0\) \OR
                  \(\X=1 \AND \x=2 \AND \y=0\)
                }{
        ...
        \ifthenelse{\X=0 \OR \X=1}{
          ...
          \draw[cB,very thick] (a20) circle (4pt);
          \draw[cC,very thick] (a11) circle (4pt);
        }

        \ifthenelse{\X=0}{
        \draw[thick,-{Stealth[length=5mm]}] (a20) -- ($(a01)+(-0.35,0.25)$) node[above=0.35cm,cA] {$x_i+(\nabla^2 f^a_i(x_i))^{-1}\nabla f^a_i(x_i)$};
