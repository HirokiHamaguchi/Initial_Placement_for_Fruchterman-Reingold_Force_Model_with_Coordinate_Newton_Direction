...
int main()
{
  ...
  {
    std::string modelStr = (model == ForceModelType::FR ? "FR" : model == ForceModelType::HC ? "HC"
                                                                                             : "Eades");
    std::cout << "Model: " << modelStr << std::endl;

    std::vector<Method> methods =
        model == ForceModelType::FR ? std::vector<Method>{Method::FR, Method::CN_FR, Method::L_BFGS, Method::CN_L_BFGS} : std::vector<Method>{Method::L_BFGS, Method::CN_L_BFGS};
          ...
          if (seed == opt_seed)
            problem.printOutput(positions,
                                "doc/main/individual/out/" + problem.matrixName + "_" +
                                    MethodStr[method] + "_" + modelStr + ".out");
        }
    ...
    std::string fileName = "doc/main/individual/hist_" + modelStr + ".txt";
