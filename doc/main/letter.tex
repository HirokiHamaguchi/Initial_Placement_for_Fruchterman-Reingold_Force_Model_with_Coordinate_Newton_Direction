...
\begin{response}
  ご指摘に感謝いたします。原稿では確かになぜ我々が FR モデルに焦点を当てたのかを十分に正当化できていませんでした。このご指摘に対し、我々は次の2点を明確にすることで、FRモデルに焦点を当てることの正当性を示したいと思います。

  まず、1点目。本研究の焦点は、Prob. (1)に対して、どのようにすればより効率的な最適化手法を開発できるか、という点にあります。恐らく、本論文の動機や主結果を誤解されていると思うので、そこを明確にします。
  ...
  そして本提案は、このProb. (1)に対して、Coordinate Newton direction+L-BFGSという最適化手法を用いれば、より大きな改善が得られることを示すものです。L-BFGSがFR modelにあまり適していないというのは、FRモデルの問題点であるというよりも、はるかにL-BFGSがFRモデルに固有の構造を活かして最適化できていないということを示唆するものになっています。
  なので、L-BFGSがFRモデルに向いていないというご指摘は、むしろ、我々の提案の重要性を強調するものであると言えます。
  この枠組みは、sec.3でより明確に示しました。

  Section 5の実験結果については、確かに、提案手法とFRアルゴリズムの組み合わせは、L-BFGSとの組み合わせほどの改善を示さないことが多いです。しかし、それは我々の主張と矛盾していません。
  ...
  続いて、2点目。ご指摘の通り、今回の枠組みは、より広くとらえれば、KKモデルなどの他の力学モデルにも適用可能なものになっています。この点については、従来の書き方は過度に限定してしまっており、そこは大きな改善を行いました。KKモデルに対しても、同様の手法を適用することで、より効率的な最適化が可能になることを示す実験も追加しました。

  その上で、FRモデルはKKモデルと比較すると、グラフの疎性が活かしやすいという特徴があります。それが、今回FRモデルに着目したもう一つの理由です。
\end{response}
...
\begin{response}
  ...
  graph isomorphism への応用に関するご指摘にも同意いたします。この応用は FR モデルに特に自然なものとは言い難く、本論文の主たる貢献から読者の注意を逸らす可能性があるため、改訂稿ではこの議論を削除しました。

  また、animationやインタラクションのような動的なグラフ描画シナリオにおいて、FRモデルのsoft behaviorが特に適していることは、非常に重要な観点だと思っています。section 6.1などでこの点に言及し、さらに、提案手法がこのようなシナリオにおいても有用である可能性について議論を追加しました。
\end{response}
...
\begin{response}
  ...
  元原稿では前者をFR force modelと呼び、後者をFR algorithmと呼んでいましたが、その誤解しやすさを鑑み、出来る限りFR force modelはthe force model of the FR algorithmやthe problem (1)などのように置き換えました。また、section 2の該当箇所についても説明を改め、誤解が生じないようにしました。
\end{response}
...
\begin{response}
  Thank you for the suggestion. We agree that the current index-based coloring does not add much interpretive value. We revise the visualization by replacing the rainbow map with a sequential continuous color map, ``viridis''.
\end{response}
...
\begin{response}
  ご指摘ありがとうございます。
  これに関しては、FR algorithmの初期段階では、stepsizeが大きすぎるために、解が振動するような現象が起きています。元々の意図としては、よりnaturalな数値実験となるように、NetworkXという外部ライブラリなどの実装と全く同じパラメータなどで実験をしていたため、意図された挙動ではありました。

  しかし、確かにこれは誤解や疑問を生じやすく、またしっかりと説明をした上であるならば、この初期stepsizeの調整は、より安定した数値実験を行うための、自然な選択肢であるとも言えます。
  そこで、改訂稿では、FR algorithmの初期段階でのstepsizeを調整して、振動を大きく抑えました。結果として、より分かりやすく提案手法の効果などが見えるようになったと思います。
\end{response}
