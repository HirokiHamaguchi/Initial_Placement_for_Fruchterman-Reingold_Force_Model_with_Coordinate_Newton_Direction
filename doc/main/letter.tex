...
そして本提案は、このProb. (1)に対して、Coordinate Newton direction+L-BFGSという最適化手法を用いれば、より大きな改善が得られることを示すものです。L-BFGSがFRにあまり適していないというのは、FRモデルの問題点であるというよりも、はるかにL-BFGSがFRモデルに固有の構造を活かして最適化できていないということを示唆するものになっています。
...
\begin{response}
  ...
  元原稿では前者をFR force modelと呼び、後者をFR algorithmと呼んでいましたが、その誤解しやすさを鑑み、出来る限りFR force modelはthe force model of the FR algorithmやthe problem (1)などのように置き換えました。また、その区別をしているということを、section 2の段階で明確にし、該当箇所についても説明を追加しました。
\end{response}
...
\begin{response}
  Thank you for the suggestion. We expanded the discussion of multilevel approaches in the related work section, and we also added a new experiment on a large graph to demonstrate the potential benefits of our method in a multilevel context.
\end{response}
