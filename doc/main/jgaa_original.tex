...
\begin{gather*}
  E^{\mathrm{a}}_{i,j}(d) \coloneqq \int_{0}^{d}F_{i,j}^{\mathrm{a}}(r) \dd{r}=
  \frac{a_{i,j}d^{3}}{3k}, \quad E^{\mathrm{r}}(d) \coloneqq \int_{1}^{d}F^{\mathrm{r}}
  (r) \dd{r}= -k^{2}\log{d}, \\
  E_{i,j}(d) \coloneqq E_{i,j}^{\mathrm{a}}(d) + E^{\mathrm{r}}(d).
\end{gather*}
For simplicity, we define $E_{i,j}(0)=\infty$.%
\footnote{We can also use $-k^{2}\log(d + \epsilon^{\mathrm{r}})$ for $E^{\mathrm{r}}
...
While the energy function $E_{i,j}(d)$ is convex and minimized when
$d = k/\sqrt[3]{a_{i,j}}$ for a positive $a_{i,j}$, a function $x_{i}\mapsto E_{i,j}
  (\norm{x_i - x_j})$ is non-convex for a fixed $x_{j}$. Additionally, $E_{i,j}$ is not Lipschitz continuous near $d=0$, as illustrated in \cref{fig:energy3d}.
...
\begin{figure}[t]
  ...
  \caption{The energy function $E_{i,j}(\norm{x_i - x_j})$ for
...
Multilevel approaches such as \textsf{sfdp} are important prior work for this FR force model~\citep{Hu2006EfficientHF}. These methods use hierarchical coarsening and refinement to achieve efficient layout computation for large graphs.
...
\begin{equation*}
  f_{i}(x_{i}) \coloneqq \sum_{j \in V \setminus \{i\}}\Big( E_{i,j}(\norm{x_{i}- x_{j}}) + E_{j,i}(\norm{x_{j}- x_{i}}) \Big).
\end{equation*}
...
$E^{\mathrm{a}}_{i,j}$ and the $\order*{\abs{V}^2}$ terms from
$E^{\mathrm{r}}$.
...
that arises from the repulsive energy $E^{r}$:
\begin{equation*}
  -\sum_{i<j}k^{2}\log{\norm{x_i - x_j}}.
\end{equation*}
...
Since $E^{\mathrm{r}}(d)=-k^{2}\log{d}$ is a convex function such that it
...
\begin{algorithm}
  ...
  Sample $x_{i}\in Q^{\mathrm{hex}}$ for all $i \in V$ without replacement\; \For{$m \gets 0$ \KwTo $N_{\mathrm{iter}}^{\mathsf{CN}}$}{ Select vertex $i \in V$ randomly\; Draw $r \in \bbR^{2}$ randomly from a unit circle\; $x_{i}^{\mathrm{new}}\gets \mathrm{round}(x_{i}- \nabla^{2}f_{i}(x_{i})^{-1}\nabla f_{i}(x_{i}) + t \cdot r)$\; \If{$\exists j \in V$ such that $x_{j}= x_{i}^{\mathrm{new}}$}{
    % Swap $x_i$ and $x_j$\;
    $x_{j}\gets x_{i}$\; } $x_{i}\gets x_{i}^{\mathrm{new}}$\; $t \gets t - t_{0}/ N_{\mathrm{iter}}^{\mathsf{CN}}$\; }
...
\begin{figure*}[p]
  \includegraphics[width=\columnwidth]{../TVCG/individual.pdf}
  \caption{
...
of the FR and L-BFGS algorithms as $N_{\mathrm{iter}}^{\mathsf{FR}}= N_{\mathrm{iter}}
    ^{\mathsf{L-BFGS}}= 200$. We tested with 7 graphs: \texttt{cycle300}, \texttt{jagmesh1},
...
Sparse Matrix Collection~\citep{davis2011university}, and these choices are based
...
For the algorithms with random initial placement, we set $N_{\mathrm{iter}}^{\mathsf{FR}}
  = N_{\mathrm{iter}}^{\mathsf{L-BFGS}}= 50$ , the same as the default parameter
in NetworkX~\citep{hagberg2008exploring}. For the \textsf{CN} algorithms, we set
$N_{\mathrm{iter}}^{\mathsf{FR}}= N_{\mathrm{iter}}^{\mathsf{L-BFGS}}= 45$,
...
Applying SGD to graph drawing, particularly to the Kamada–Kawai (KK) layout~\citep{kamadaAlgorithmDrawingGeneral1989}, has been a notable direction~\citep{zhengGraphDrawingStochastic2019,ahmedMulticriteriaScalableGraph2021}. The KK model determines the layout by minimizing a stress-like energy $E_{i,j}^{\mathrm{KK}}$ defined for all vertex pairs. SGD corresponds to randomly selecting an edge ${i,j}$ and updating $x_i$ and $x_j$ using the gradient of $E_{i,j}^{\mathrm{KK}}$. This stochastic treatment reduces per-iteration complexity while retaining structural information, as $E_{i,j}^{\mathrm{KK}}$ is defined in terms of graph distance.

In contrast, applying SGD directly to the FR model is ineffective. When $a_{i,j}=0$, the energy term reduces to $E_{i,j}=-k^{2}\log d$, which depends only on Euclidean distance and ignores the graph structure. Consequently, gradients from a single sampled pair often fail to provide meaningful information for optimization.

Another approach is to use Stochastic Coordinate Descent, which resembles Randomized Subspace Newton methods~\citep{NEURIPS2019_bc6dc48b, fujiRandomizedSubspaceRegularized2022, cartisRandomisedSubspaceMethods2022, nozawaRandomizedSubspaceGradient2023, higuchiFastConvergenceSecondOrder2024}.
For each vertex $i$, define the local energy
\begin{equation}\label{eq:fi}
  f_{i}(x_{i}) \defeq \sum_{j \in V \setminus \{i\}}E_{i,j}(\norm{x_i - x_j}),
\end{equation}
...
\begin{mini*}
  {X \in \bbR^{2 \times n}} {f(X) = \sum_{\{i,j\}\in E} f_{i,j}(x_i, x_j)+\lambda \sum_{i=1}^{n} \Omega_i(x_i),}
  {} {}
\end{mini*}
...
optimization problem on Riemannian manifolds~\citep{klusContinuousOptimizationMethods2023},
...
\begin{equation*}
  f_{i}(x_{i}) \coloneqq \sum_{j \in V \setminus \{i\}}\left( E_{i,j}(\norm{x_{i}- x_{j}}
  ) + E_{j,i}(\norm{x_{j}- x_{i}}) \right).
\end{equation*}
