...
\begin{abstract}
  ...
  We also discuss \red{applications of our method, such as drawing for Eades or Hooke--Coulomb force models.}
\end{abstract}
...
\red{A central challenge in force-directed graph drawing is efficiently obtaining an equilibrium state of the force system.
  ...
  This refinement remains important even in} multilevel approaches such as the Scalable Force-Directed Placement (\textsf{sfdp}) algorithm~\citep{Hu2006EfficientHF,ellsonGraphvizOpenSource2002}, where layouts must still be refined while propagating solutions from coarse to fine graph levels. See also \cref{sec:application_like_multilevel} for more discussion.

\red{Two major approaches are commonly used for refinement.
  The first is direct force simulation \citep{fruchtermanGraphDrawingForcedirected1991}, which repeatedly applies the forces to vertices to approximate equilibrium.
  The second is numerical optimization based on explicit energy minimization. In particular, the L-BFGS (Limited-memory Broyden--Fletcher--Goldfarb--Shanno) algorithm~\citep{liuLimitedMemoryBFGS1989a}}, a general-purpose optimization method, has been reported to produce high-quality layouts for several force models~\citep{6183577}, including the FR force model and Kamada--Kawai (KK) model~\citep{kamadaAlgorithmDrawingGeneral1989}.
\red{Although effective, both approaches require many computationally expensive iterations. When the optimization is terminated early, the resulting layouts may contain twists, including folded or tangled structures that reduce readability}, as shown in \cref{fig:fig1}~\cite{veldhuizenDynamicMultilevelGraph2007,cheongSnapshotVisualizationComplex2018}.

\red{One possible approach to mitigating the issue is to improve the initial placement before refinement.
  Both simulation-based algorithm and L-BFGS algorithm involve relatively expensive iterations, it is often more efficient to first move the layout toward a reasonably good configuration using inexpensive approximate updates, and then apply full-cost refinement methods only after the layout becomes sufficiently close to equilibrium. In other words, this pre-processing substitutes for part of the refinement while reducing the overall optimization difficulty.}
...
\red{However, the existing method assumes circular initial layouts and employs a relatively heuristic objective function, insufficiently grounded in the underlying force model. In addition, the method mainly targets unweighted, small-scale, and structurally simple graphs, limiting its applicability to more general graph drawing settings.}

\red{In this paper, we propose a new initial placement to quickly obtain the equilibrium layout of the force models, especially the FR force model.} % LLDisable
...
\begin{enumerate}[label=\textbullet, itemsep=0pt, parsep=0pt]
  \item In \cref{sec:preliminaries}, \red{we provide the technical definitions of the problem.}
  ...
  \item In \cref{sec:discussion}, we discuss our approach and its potential applications.
...
\red{In this study, we adopt an energy-based optimization approach for graph drawing. While Fruchterman and Reingold~\citep{fruchtermanGraphDrawingForcedirected1991} originally proposed the force-directed simulation-based algorithm based on a physical particle system, energy-based methods achieve the same goal by directly minimizing an energy function $\Phi$. These two perspectives are fundamentally equivalent because} a local minimum of the energy function \red{$\Phi(X)$} yields an equilibrium position since \red{$\nabla \Phi(X)$} corresponds to the forces. In this sense, the force-based and energy-based perspectives are equivalent as both capture equilibrium, illustrated in \cref{fig:frLayout}.
...
\red{Let us formally define the energy-minimization problem.}
Let $G = (V, E)$ be an undirected graph with vertex set $V = \{1, \dots, n\}$ and edge set $E$.
Each edge $\red{(i,j)} \in E$ has positive weight $a_{i,j} = a_{j,i} \in \mathbb{R}$.
...
\end{equation*}
For simplicity, we define $\Phi^{\mathrm{r}}_{i,j}(0)=\infty$.%
...
\begin{mini}
  ...
  {\label{prob:fr}} {}
\end{mini}
\red{We will call Problem~\ref{prob:fr} as the energy-minimization problem for the FR force model.}

We also define the vertex pair wise energy function $\red{\Phi}_{i,j}(d) \coloneqq \red{\Phi}_{i,j}^{\mathrm{a}}(d) + \red{\Phi}^{\mathrm{r}}(d)$ for convenience.
While the energy function $\Phi_{i,j}(d)$ is convex and minimized when $d = k/\sqrt[3]{a_{i,j}}$ for a positive $a_{i,j}$,
a function $x_{i}\mapsto \Phi_{i,j} (\norm{x_i - x_j})$ is non-convex for a fixed $x_{j}$.
...
Optimization-based methods for graph drawing have been actively investigated. Representative approaches include GPU acceleration~\citep{gajdosParallelFruchtermanReingold2016}, machine learning techniques~\citep{tiezziGraphNeuralNetworks2024}, multi-objective formulations~\citep{ahmedMulticriteriaScalableGraph2022a}, and stochastic gradient descent~\citep{zhengGraphDrawingStochastic2019}.
Among these, our attention is on the underlying optimization algorithms itself.
...
\end{equation*}
\red{Here, $\Phi_{i,j}(\cdot)=\Phi_{j,i}(\cdot)$ holds for all $i,j\in V$.}
...
\red{\begin{align*}
    (\nabla \Phi(X))_{i} & \defeq \nabla \Phi_{i}(x_{i}) = \sum_{j \in V \setminus \{i\}}\left(\frac{a_{i,j}\norm{ x_{i}- x_{j}}}{k}- \frac{k^{2}}{\norm{ x_{i}- x_{j}}^{2}}\right) (x_{i}- x_{j}) \\
                         & = \sum_{j \in V \setminus \{i\}}\left(\frac{a_{i,j}\norm{ x_{i}- x_{j}}}{k}- \frac{k^{2}}{\norm{ x_{i}- x_{j}}^{2}}\right) (x_{i}- x_{j}).
  \end{align*}}
\red{Since both the objective value and gradient are explicitly available, a variety of gradient-based optimization methods can be applied.
  The classical simulation-based algorithm may be interpreted as a steepest-descent-type method in which the step size is controlled by a cooling schedule. Indeed, $(\nabla \Phi(X))_i$ corresponds to the force acting on vertex $i$.}
The L-BFGS algorithm, a renowned and widely used optimization method, is another first-order gradient-based method that has been shown to
consistently outperforms the original simulation-based algorithm in speed and layout quality~\citep{6183577}. A common choice for the initial solution is a random placement of nodes within the bounding box
$\{(x_{i}^{(1)},x_{i}^{(2)}) \mathrel\mid 0 \leq x_{i}^{(1)}\leq 1, 0 \leq x_{i}^{(2)}\leq 1\}$~\citep{2020SciPy-NMeth}.

However, when the input is highly twisted or otherwise ill-conditioned, both the simulation-based algorithm and the L-BFGS algorithm can suffer from large per-iteration costs: each iteration operates on the full set of coordinates, which limits its ability to untangle local geometric defects.
\red{Thus, to address these issues, we need an alternative optimization approach that differs from both the simulation-based algorithm and the L-BFGS algorithm. As such an approach, this study proposes stochastic coordinate descent with coordinate Newton directions, a second-order optimization method used as preprocessing for first-order gradient-based methods.}

\subsection{Initial Placement Approach}
\label{ssec:preprocessing}

A preprocessing step with Simulated Annealing (SA) is known to be effective~\citep{ghassemitoosiSimulatedAnnealingPreProcessing2016} since SA can avoid getting stuck in local optima and leads to better visualization, combined with the simulation-based algorithm. Let
\begin{equation*}
  Q^{\mathrm{circle}} \defeq \left\{(\cos(2\pi k/n), \sin(2\pi k/n)) \mid 1 \leq k \leq n \right\}
\end{equation*}
...
the \red{energy minimization problem}.
This study obtains a faster and better visualization by setting the
result of Simulated Annealing (SA) for Problem~\eqref{eq:sa} as an initial
placement for the simulation-based algorithm.
...
the energy minimization problem. The algorithm solves a discrete optimization problem with stochastic coordinate descent to find an initial placement with fewer twists.
...
to the \red{energy minimization problem}.
We can compute the coordinate
Newton direction by taking the position $x_{i}$ of the vertex $i$ as the coordinate
block.
Although directly applying this idea to the \red{energy minimization problem} is challenging,
...
\begin{mini}
  {X \in \bbR^{2 \times n}} {\Phi^\mathrm{a}(X) \defeq \sum_{\red{(i,j)}\in E} \frac{a_{i,j}\norm{x_i - x_j}^{3}}{3k},}
...
\begin{figure}[t]
  ...
  \caption{ Concept of $Q$. The assignment from $V$ to a discrete point
    ...
    placement for Problem~\eqref{eq:frApprox0}. }
  \label{fig:pi}
\end{figure}

\red{Let us explain the rationale for this formulation.}
First, the \red{energy minimization Problem~\eqref{prob:fr}} is equivalent to the following:
\begin{mini*}
...
$\Phi^{\mathrm{r}}$.
Here, following previous research mentioned in \cref{ssec:preprocessing}, we fix the possible positions $x_{i}$ of each vertex $i$ to a discrete set of points $Q$, where $\abs{Q}\geq \abs{V}$.
...
Since $\Phi^{\mathrm{r}}(d)=-k^{2}\log{d}$ is a convex function such that it decreases monotonically concerning $d$, for sufficiently large $d$, the value of $-k^{2}\log{d}$ does not vary excessively.
...
defined by~\cref{eq:qHex} (see \cref{fig:pi}) as $Q$.
...
In this context, the hexagonal lattice is known for its densest packing structure in space with the least distance $\epsilon$ between points \citep{changSimpleProofThues2010} and offers
...
This is a large difference from the functions $\Phi(X)$ in the problem \eqref{prob:fr} and $\Phi^{\mathrm{a}} (X)$ in Problem~\eqref{eq:frApprox0}, which are non-convex.
...
Problem~\eqref{eq:frApprox0}. Thus, we project this $x_{i}^{\mathrm{new}}$
...
\end{equation*}
where $\mathrm{round}(\cdot)$ denotes the operation assigning a point in $\mathbb{R}^2$ to the nearest point in the hexagonal lattice $Q^{\mathrm{hex}}$, $r$ is a random vector with a decreasing magnitude.
...
positions $x_{i}$ and $x_{j}$ to satisfy the condition $x_{i}\neq x_{j}$. Otherwise, we just update $x_{i}$ to $x_{i}^{\mathrm{new}}$.
...
\end{equation}
This value can be computed in $\order{\abs{E}}$ complexity, enabling us to obtain a better initial placement.
Notably, the optimal solution to Problem~\eqref{eq:frApprox0} is invariant
...
\begin{algorithm}[t]
  ...
  \label{alg:proposed} \KwIn{Graph $G = (V, E)$, Weight $(a_{i,j})_{\qty{i,j} \in E}$, Parameter $N_{\mathrm{iter}}^{\mathsf{CN}}\in\bbN$.}
...
We state the application of our proposed method to other force models.
Although we have focused only on the FR force model, the concept of our approach can be applied to other models, as long as the objective function has a similar structure to $\Phi_{\mathrm{FR}}(X)$.
Recall that the energy minimization problem \eqref{prob:fr} can be rewritten as
\begin{mini*}
...
In general, this type of optimization problems are more broadly treated as
``objective functions arising from graphs.''~\citep{wright2022optimization}
...
The Hessian matrix for the coordinate Newton direction can be indefinite in these models, which means the Newton direction may not be a descent direction.
Still, there are various remedies for this issue in the optimization literature, such as adding a damping term or using a regularization to ensure the positive definiteness \citep{uedaRegularizedNewtonMethod2014a,nocedal1999numerical}.
Thus, we can still leverage the coordinate Newton direction for these models almost in the same way as for the FR model.

We clarify that the proposed method is not necessarily effective for all objective functions arising from graphs, and there are some cases where it may not be suitable.
One representative example in graph drawing for which the framework of the proposed method is not well suited is its application to the KK model~\citep{kamadaAlgorithmDrawingGeneral1989}.
The objective function of the KK model is as follows:
\begin{mini*}
  {X \in \bbR^{2 \times n}} {\Phi_{\mathrm{KK}}(X) \coloneqq \sum_{\substack{(i,j) \in V^2 \ i \neq j}}  \frac{(\lVert x_{i} - x_{j} \rVert - l_{ij})^2}{2 l_{ij}^2}}
  {} {}
\end{mini*}
Indeed, this objective function is clearly different from those discussed above: it consists only of terms over all pairs of vertices, and there is no principal term defined solely by the edges.
Since our approach computes the Hessian matrix for the terms defined by the edges, it is not directly applicable to the KK model.
More generally, although it is possible to compute the coordinate Newton direction directly for this problem, its behavior is not necessarily favorable. See also \cref{sec:rationale}.

For the KK model, the use of an optimization method called SGD has been proposed~\citep{zhengGraphDrawingStochastic2019,ahmedMulticriteriaScalableGraph2022a}, and such methods are considered more likely to perform well.
Briefly, this approach repeatedly selects a pair of vertices and optimizes with respect to that pair.
In this sense, it can be regarded as complementary to the present method, which repeatedly selects a single vertex and optimizes with respect to that vertex.
}
...
with g++ 13.3.0.
\red{The experiments were run on a Dynabook CZ/MWS laptop equipped with a 13th Gen Intel Core i7-1360P processor and 16 GB of RAM. The software environment was Ubuntu 24.04 LTS under Windows Subsystem for Linux 2 on Windows 11 Home 64-bit.}

For a fair comparison, we implemented the \red{simulation-based} algorithm in \Cpp{} based on
...
library for the L-BFGS algorithm.
...
\end{enumerate}
As an algorithm to solve the \red{energy minimization problem}, we used
\begin{enumerate}[label=\textbullet, itemsep=0pt, parsep=0pt]
  \item the simulation-based algorithm (\textsf{SIM}),

  \item L-BFGS algorithm (\textsf{L-BFGS}).
\end{enumerate}
\red{The key remark is that we slightly modified the original simulation-based algorithm so that it automatically adjust the stepsize. This is to supress overshoot and isolate the effect of the proposed initialization from the effect of the stepsize. This is just done by a simple backtracking line search at the first iteration.}

As parameters, \green{we used initial temperature $t_{0}=1.5$} and the number of
...
\begin{figure*}[p]
  ...
  \caption{
    Experimental results for the FR force model. Our proposed method \textsf{CN}, Coordinate Newton, yields improved results for both the simulation-based algorithm and L-BFGS.
...
\begin{figure*}[p]
  ...
  \caption{
    \red{Experimental results for the HC and Eades force models.
      Our proposed method \textsf{CN}, Coordinate Newton, yields improved results for both the simulation-based algorithm and L-BFGS.
      The color of vertices represents their indices.}
  }
  \label{fig:individual_HC_and_Eades}
\end{figure*}

We first show the plots and visualizations of the algorithms in \cref{fig:individual}.
The experiment details are as follows. We fixed the maximum number of iterations of the FR and L-BFGS algorithms as $N_{\mathrm{iter}}^{\mathsf{SIM}}= N_{\mathrm{iter}}^{\mathsf{LBFGS}}= \red{150}$.
...
\red{We provide more visualization results for large graphs in \cref{app:large_graphs}.}
...
Secondly, the initial values of $\Phi(X)$ with \textsf{CN-SIM} are significantly smaller than those with \textsf{SIM}, suggesting that the proposed method provides a good initial placement.
Thirdly, the visualization results also support the effectiveness of the proposed initial placement. In most cases, placements obtained with \textsf{CN} better reflect the nearly optimal arrangement.

As a side note, regardless of \textsf{CN} or non-\textsf{CN}, the algorithms with \textsf{L-BFGS} consistently outperform those with \textsf{SIM}.
...
\red{As we have mentioned in \cref{sec:otherForceModels}, the proposed algorithm can be applied to other force models, such as the HC and Eades models.
  The main difference from the FR model is that the number of iterations for the subsequent optimization algorithms such as L-BFGS, not our proposed method, was increased.
  Since these models are more difficult to optimize than the FR model, we set $N_{\mathrm{iter}}^{\mathsf{SIM}}= N_{\mathrm{iter}}^{\mathsf{LBFGS}}= 400$ for these models.
  We kept the other settings the same as those for the FR model, including the parameters of the proposed algorithm.
  We showed the results for these models in \cref{fig:individual_HC_and_Eades}.
  The proposed algorithm also performed better than random and SA initialization for these models.}
...
For the algorithms with random initial placement, we set $N_{\mathrm{iter}}^{\mathsf{SIM}}
...
$N_{\mathrm{iter}}^{\mathsf{SIM}}= N_{\mathrm{iter}}^{\red{\mathsf{LBFGS}}}= 45$,
...
\begin{figure*}[t]
  ...
  \begin{tabular}{cccccc}
    ...
    \cmidrule(lr){2-3} \cmidrule(lr){4-5} \rotatebox{90}{\quad \textsf{SA}} & \includegraphics[width=0.175\columnwidth]{circle/vis/dwt_992_SA-L-BFGS_50_first.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/dwt_992_SA-L-BFGS_50_last.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/collins_15NN_SA-L-BFGS_50_first.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/collins_15NN_SA-L-BFGS_50_last.png}   \\
    \addlinespace \rotatebox{90}{\quad \textsf{CN} (proposed)}              & \includegraphics[width=0.175\columnwidth]{circle/vis/dwt_992_CN-L-BFGS_50_first.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/dwt_992_CN-L-BFGS_50_last.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/collins_15NN_CN-L-BFGS_50_first.png} & \includegraphics[width=0.175\columnwidth]{circle/vis/collins_15NN_CN-L-BFGS_50_last.png}   \\
    \bottomrule
  \end{tabular}
...
\red{In a typical multilevel pipeline, a graph is recursively coarsened, an initial layout is computed at the coarsest level, and the layout is then refined after successive uncoarsening steps using the simulation-based approach.}

\red{
  Our work is complementary to multilevel layout methods.
  The proposed method could be incorporated into the pipeline in several ways.
  A simple possibility is to use it at the coarsest level, or at selected intermediate levels, as an alternative to random initialization.
  Another possibility is to use it during refinement: after a coarse vertex is expanded into a set of finer vertices, the proposed initialization could be applied locally to the corresponding induced subgraph.
  In such a setting, the local placement should be constrained by the position of the parent coarse vertex and by the neighboring coarse-level structure so that inter-subgraph edges remain consistent with the global layout.
}

\red{
  The proposed method may also be combined with numerical optimization techniques.
  For example, simulation-based refinement steps could be replaced by, or augmented with, optimization methods such as L-BFGS, using the proposed placement as an initial solution.
  Although our implementation uses a hexagonal lattice to define candidate positions, the essential requirement is not necessarily the lattice structure itself, but rather the availability of sufficiently separated candidate positions.
  This suggests that similar initialization or coordinate-wise minimization strategies could be applied after solving a coarsened graph, either locally within expanded vertex groups or globally over the entire graph.
...
\red{
  The proposed approach may also be relevant to dynamic graph layouts, including animation and interactive visualization.
  In real-time settings, especially for large graphs, inexpensive update steps are important.
  Because the proposed method has lower per-iteration cost than the standard FR algorithm in our setting, it may be useful for initializing newly introduced vertices, improving local configurations, or accelerating refinement within scalable simulation-based dynamic layout methods.
  However, preserving temporal coherence is a central requirement in dynamic graph drawing, and therefore the integration of the proposed method with dynamic layout algorithms should be investigated carefully.
}
...
One natural idea for solving the energy minimization problem is to apply the stochastic coordinate descent, which resembles Randomized Subspace Newton methods~\citep{NEURIPS2019_bc6dc48b, fujiTheoreticalAnalysisRandomized2025, cartisRandomisedSubspaceMethods2022, nozawaRandomizedSubspaceGradient2023, higuchiFastConvergenceSecondOrder2024}.
...
Set $k = 1/2$ and assign all positive edge weights $a_{i,j}= 1$ for
...
Our proposed method resolves issues in \Cref{ssec:inaccuracy,ssec:ignorance} by transforming Problem~\eqref{prob:fr} into Problem~\eqref{eq:frApprox0}
...
\red{
  We also conducted experiments on several additional graphs, and the results are shown in \cref{fig:individual_2}.
  For the choice of graphs, we referred to \citep{arleoDistributedMultilevelForceDirected2019,bartelExperimentalEvaluationMultilevel2011} and selected seven graphs:
  ``cylinder\_30\_30'', ``gr\_30\_30'', ``sierpinski\_06'', ``jagmesh8'', ``USpowerGrid'', ``wiki-Vote'', and ``crack''.
...
\red{We observe a trend similar to that in \cref{fig:individual}. The proposed method, \textsf{CN}, improves the results for both the simulation-based algorithm and L-BFGS.}
...
\red{
  ...
  For large-scale graphs with well over ten thousand vertices, it will likely be necessary to combine the proposed method with a multilevel approach, as discussed in \cref{sec:application_like_multilevel}.
}
...
\begin{figure*}[p]
  ...
  \caption{
    \red{Experimental results for FR force model with larger graphs.
      Our proposed method \textsf{CN}, Coordinate Newton, yields improved results for both the simulation-based algorithm and L-BFGS.
      The color of vertices represents their indices.}
  }
...
\begin{algorithm}[H]
  ...
  \label{alg:fr} \KwIn{Graph $G = (V, E)$, Weights $(a_{i,j})_{(i,j) \in E}$, Parameters $N_{\mathrm{iter}}^{\mathsf{SIM}}\in \bbN$, $t_{0}> 0$, and Initial~placement~$X = (x_{1}, \dots, x_{n})$}
  \KwOut{Final placement $X$}

  $t \gets t_{0}$\; \For{$m \gets 1$ \KwTo $N_{\mathrm{iter}}^{\mathsf{SIM}}$}{ $\text{compute gradient }\nabla \Phi_{i}(x_{i})$ for all $i \in V$\; $x_{i}^{\mathrm{new}}\gets x_{i}- t \frac{\nabla \Phi_{i}(x_{i})}{ \norm{\nabla \Phi_i(x_i)}}$ for all $i \in V$\; $x_{i}\gets x_{i}^{\mathrm{new}}$\; $t \gets t - t_{0}/ N_{\mathrm{iter}}^{\mathsf{SIM}}$\; \If{\text{convergence condition is satisfied}}{ \textbf{break}\; } }
  \Return $X$\;
\end{algorithm}
...
\begin{figure}[t]
  ...
  \caption{Comparison of the simulation-based and L-BFGS algorithms. Blue arrows represent fixed stepsizes in FR, whereas orange arrows represent adaptive stepsizes determined by the approximate inverse Hessian}
...
Hessian of the objective function $\Phi$~\citep{liuLimitedMemoryBFGS1989a}. L-BFGS is known to be very efficient for large-scale optimization problems.
...
The problem setting considered in NetworkX is similar to that described in~\cref{sec:preliminaries}, but there are some key differences. First, the number of dimensions is not always two.
...
It means that the optimal solution does not exist in a finite region. When edge weights are negative or the graph is disconnected, i.e., when it consists of $m >1$ connected components ($V=\bigoplus_{j=1}^{m}V_{j}$), the solution may be unbounded, since the farther apart the vertices are, the lower the energy.
