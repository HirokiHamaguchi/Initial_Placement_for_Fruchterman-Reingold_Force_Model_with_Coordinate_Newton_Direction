...
It means that we consider the following:
\begin{mini*}
  {X \in \bbR^{2 \times n}} {\sum_{\{i,j\}\in E} \frac{a_{i,j}\norm{x_i - x_j}^{3}}{3k}- \sum_{i<j} k^2\log{\norm{x_i - x_j}},}
...
To leverage this sparsity for simplification, we want to drop the second term that arises from the repulsive energy $\Phi^{r}$:
\begin{equation*}
  -\sum_{i<j}k^{2}\log{\norm{x_i - x_j}}.
\end{equation*}
...
\begin{mini*}
  ...
  \addConstraint{x_i}{\neq x_j \quad}{\text{for $1 \leq i < j \leq n$}.}
\end{mini*}
This means that we skip to consider the $\order*{\abs{V}^2}$ pairs (all pairs of $\{i,j\}$ with $1 \leq i < j \leq n$) by fixing the possible point placement in advance, reducing the computational complexity to $\order{\abs{E}}$ and thus offering significant speedup.
...
This is a large difference from the functions $\Phi(X)$ in the problem \eqref{prob:fr} and $\Phi^{\mathrm{a}} (X)$ in the problem~\eqref{eq:frApprox0}, which are non-convex.
...
Although we have focused only on the FR force model, as it clear from the previous discussion, the concept of our approach can be applied to a wider range of force models, as long as the objective function has a similar structure to $\Phi_{\mathrm{FR}}(X)$.
...
Thus, our proposed approach can be applied to these force models as well, and we can compute the coordinate Newton direction for each vertex in these models.
Although the Hessian matrix for the coordinate Newton direction can be indefinite in these models, which means the Newton direction may not be a descent direction.
See also \cref{sec:rationale}.
Still, there are various remedies for this issue in the optimization literature, such as adding a damping term to the Hessian matrix or using a regularization term in the objective function to ensure the positive definiteness of the Hessian matrix.
Thus, we can still leverage the coordinate Newton direction for these models. \green{todo: cite}

しかし、全てのグラフから生じる目的関数に対して、提案手法が有効だとは限りません。
提案手法の枠組みが向かないものの、グラフ描画の代表例として挙げられるものとして、KK model~\citep{kamadaAlgorithmDrawingGeneral1989}に対する適用が考えられます。
...
\begin{mini*}
  ...
  {} {}
\end{mini*}
実際、これは先ほどまでにあげた目的関数とは明確に異なり、すべての頂点ペアに対する項のみが存在し、辺だけによる主要項というのが存在しません。その為、directにproposed methodを適用することは難しいと考えられます。より一般に、この問題に対して直接coordinate Newton directionの計算をすることは可能ですが、その挙動は必ずしも良いものになるとは限りません。See also \cref{sec:rationale}.
KK modelに対しては、SGDと呼ばれる最適化手法の活用が提案されており\citep{zhengGraphDrawingStochastic2019,ahmedMulticriteriaScalableGraph2022a}、そのような手法の方が上手くいくと考えられます。この手法は端的に言えば、2つの頂点対を一つ取り出して最適化を繰り返すことを旨としており、一つの頂点を取り出して最適化を繰り返すことを旨とする本手法とは補完的な関係にあると言えます。
}
...
\end{enumerate}
\red{The key remark is that we slightly modified the original simulation-based algorithm so that it automatically adjust the stepsize. This is to isolate the effect of the proposed initialization from the effect of the stepsize. This is just done by a simple backtracking line search at the first iteration.}
...
of the FR and L-BFGS algorithms as $N_{\mathrm{iter}}^{\mathsf{SIM}}= N_{\mathrm{iter}}^{\mathsf{LBFGS}}= 200$.
...
\red{簡潔に、この手法はmultilevel approachと組み合わせる方向性について議論する。}
Multilevel approaches such as \textsf{sfdp}~\citep{Hu2006EfficientHF} in Graphviz are important prior work for obtaining the equilibrium layout of the force model, i.e., solving the energy minimization problem.
These methods use hierarchical coarsening and refinement to achieve efficient layout computation for large graphs.

\red{Our work is complementary to such multilevel methods.
  First, in the refinement step in multilevel approaches, the simulation-based algorithm is often used. It is straitforward to apply optimization techniques such as L-BFGS instead of the simulation-based approach, and the proposed method can be used as an initial placement for this optimization.
  最も簡易的な適用方法の一つは、coarseningが完全に行われて、ランダムな初期配置から計算を始める際に、提案手法をそのまま用いることは、当然適用方法の一つとして考えれられる。
  また、各レベルでのrefinementが済んで、coarseningを解いた直後に、それまで同じグループに割り当てられていた頂点集合というのが複数存在するが、それぞれのsub graphに対して直接提案の初期配置を用いて簡易的に最適化を施すことも挙げられる。
  更に、提案手法はhexagonal latticeを用いて配置していたが、これは必ずしもhexagonal latticeである必要はなく、頂点の位置の候補となる点集合が互いに十分な距離離れていれば、十分にこの手法は機能すると考えている。
  よって、coarseningをと解いた直後に、グラフ全体に対しても、本手法と同様に、先にcoordinate Newton directionを用いてenergy minimizationを行うことも考えられる。
  具体的な実装方法や、細かい工夫、どのような場合に最も効果的かなどは非自明な問いであり、本研究の更なる拡張の方向性の一つとして考えられるだろう。
...
\red{また、The FR model is usually used for dynamic graph layouts like animation and interaction.
  特に、大規模なグラフ描画をリアルタイムに行う必要があるシナリオなどでは、per iteration costがFR algorithmに比べても低い、提案手法は有用であると考えられる。
  combining the proposed algorithm with a scalable simulation-based algorithm is a promising direction for future work.}
...
is known to be very efficient for large-scale optimization problems.
We can apply the L-BFGS algorithm via flattening
the matrix $X \in \bbR^{2 \times n}$ to a vector $\overline{X}\in \bbR^{2n}$.
...
\red{We also conducted experiments on larger graphs, and the results are shown in \cref{fig:individual_2}.}
