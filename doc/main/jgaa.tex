...
Each edge $\{i,j\} \in E$ has positive weight $a_{i,j} = a_{j,i} \in \mathbb{R}$.
...
\begin{mini}
  {X \in \mathbb{R}^{2 \times n}} {\Phi(X) \coloneqq \red{\sum_{(i,j) \in E} \Phi_{i,j}^{\mathrm{r}}(\norm{x_i - x_j}) + \sum_{i<j} \Phi_{i,j}^{\mathrm{a}}(\norm{x_i - x_j})}.}
  {\label{prob:fr}} {}
\end{mini}
...
\red{
  ...
  ``cylinder_30_30'', ``gr_30_30'', ``sierpinski_06'', ``jagmesh8'', ``USpowerGrid'', ``wiki-Vote'', and ``crack''.
