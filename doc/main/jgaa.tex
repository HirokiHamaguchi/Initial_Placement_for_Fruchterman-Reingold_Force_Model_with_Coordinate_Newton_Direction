...
We can use both FR and L-BFGS algorithms to obtain the final placement.
...
\begin{gather*}
  ...
  \red{\Phi}_{i,j}(d) \coloneqq \red{\Phi}_{i,j}^{\mathrm{a}}(d) + \red{\Phi}^{\mathrm{r}}(d).
\end{gather*}
...
\begin{mini}
  {X \in \mathbb{R}^{2 \times n}} {\Phi(X) \coloneqq \red{\sum_{i < j} {}} \Phi_{i,j}(\norm{x_i - x_j}).}
  {\label{prob:fr}} {}
\end{mini}
...
\red{Problem~\eqref{prob:fr} can be interpreted as a continuous optimization problem.
...
\red{\begin{align*}
  ...
  \end{align*}}
\red{Since both the objective value and gradient are explicitly available, a variety of gradient-based optimization methods can be applied to problem~\eqref{prob:fr}.
  The classical FR algorithm may be interpreted as a steepest-descent-type method in which the step size is controlled by a cooling schedule. Indeed, $(\nabla \Phi(X))_i$ corresponds to the force acting on vertex $i$.}
...
consistently outperforms the original FR algorithm in speed and layout quality~\citep{6183577}. A common choice for the initial solution is a random placement of nodes within the bounding box
$\{(x_{i}^{(1)},x_{i}^{(2)}) \mathrel\mid 0 \leq x_{i}^{(1)}\leq 1, 0 \leq x_{i}^{(2)}\leq 1\}$~\citep{2020SciPy-NMeth}.

However, when the input is highly twisted or otherwise ill-conditioned, both FR algorithm and the L-BFGS algorithm can suffer from large per-iteration costs: each iteration operates on the full set of coordinates, which limits its ability to untangle local geometric defects.
\red{Thus, to address these issues, we need an alternative optimization approach that differs from both the FR algorithm and the L-BFGS algorithm. As such an approach, this study proposes stochastic coordinate descent with coordinate Newton directions, a second-order optimization method used as preprocessing for first-order gradient-based methods.}
...

  {
    ...
    \begin{mini*}
      {X \in \bbR^{2 \times n}} {\Phi_{\mathrm{Eades}}(X) =
        ...
        \frac{k_2}{\lVert x_{i} - x_{j} \rVert + \epsilon^{\mathrm{r}}}}{}{}
    \end{mini*}
    Clearly, these objective functions are also non-convex and have a similar structure to $\Phi_{\mathrm{FR}}(X)$, which consists of a sum of terms associated with edges and a sum of terms associated with all pairs of vertices.

    The coordinate Newton direction can be computed for each vertex in these models as well, and the stochastic coordinate descent approach can be applied to optimize the layout efficiently.
    ...
    このような場合は、恐らくSGDを用いた方が良いです。\citep{zhengGraphDrawingStochastic2019}
  }
...
\red{We provide more visualization results in appendix, including more large graphs. \citep{bartelExperimentalEvaluationMultilevel2011}}

\subsection{Results for Other Force Models}

\red{先述の通り、提案手法は他のforce modelにも適用可能である。(この結果は本文に載せるべきか?)}

\section{Discussion}
\label{sec:discussion}

\subsection{\red{Application to ???}}
\label{sec:application_like_multilevel}

簡潔に、この手法はmultilevel approachと組み合わせることができることを説明する。
...
In contrast, applying SGD directly to the FR force model is ineffective. % LLDisable
...
\begin{algorithm}[H]
  ...
  $t \gets t_{0}$\; \For{$m \gets 1$ \KwTo $N_{\mathrm{iter}}^{\mathsf{FR}}$}{ $\text{compute gradient }\nabla \Phi_{i}(x_{i})$ for all $i \in V$\; $x_{i}^{\mathrm{new}}\gets x_{i}- t \frac{\nabla \Phi_{i}(x_{i})}{ \norm{\nabla \Phi_i(x_i)}}$ for all $i \in V$\; $x_{i}\gets x_{i}^{\mathrm{new}}$\; $t \gets t - t_{0}/ N_{\mathrm{iter}}^{\mathsf{FR}}$\; \If{\text{convergence condition is satisfied}}{ \textbf{break}\; } }
  \Return $X$\;
\end{algorithm}
