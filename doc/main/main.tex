...
\title{
    ...
    \red{with Subspace Newton Direction}
}
\author{
    Hiroki Hamaguchi\,\orcidlink{0009-0005-7348-1356}
    Naoki Marumo\,\orcidlink{0000-0002-7372-4275}
    Akiko Takeda\,\orcidlink{0000-0002-8846-4496}\red{$\rightarrow$???}
    % <-this % stops a space
}
\maketitle
...
We used the 4 algorithms: FR algorithm (\textsf{FR}), the proposed initialization plus FR algorithm (\textsf{SN-FR}), L-BFGS algorithm (\textsf{L-BFGS}), and the proposed initialization plus L-BFGS algorithm (\textsf{SN-L-BFGS}). \textsf{SN} represents the Subspace Newton, and we refer \textsf{SN-FR} and \textsf{SN-L-BFGS} as \textsf{SN}, and \textsf{FR} and \textsf{L-BFGS} as non-\textsf{SN}.

As a parameter, we used $N_\mathrm{iter}^\mathsf{SN} = 3 \frac{\abs{V}^3}{\abs{E}}$ for the FR algorithm. Since the amortized time complexity per iteration of
