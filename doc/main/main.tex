...
This makes L-BFGS particularly suitable for high-dimensional optimization tasks. Since it is a more sophisticated method than the gradient descent method, the superior performance of the FR algorithm is reported in~\cite{6183577}.
...
% まず、既存の手法における課題を観察し整理する。
% いくつかの実験及び先行研究の報告を基にすると、FR layoutに対する最適化を考える上での最大の難点は、"ねじれ"の解消であると我々は考えている。

% 本論文におけるねじれとは、数学的に厳密な語ではなく、図示した際に現れる明らかに不必要な線分の交差や、本来円環状などの簡潔な構造が絡まっているように見える状況を指している。
% ねじれという言葉の用法自体は比較的少ないと認識しているが、例えば\cite{cheongSnapshotVisualizationComplex2018}などでもこの言葉が用いられている。
% 図~\ref{fig:circle}などに示したような状況が具体例である。

% ねじれの有無は、最適化の効率に大きな影響を及ぼすと我々は考えている。
% 図~\ref{fig:circle}の状況において、頂点がぐちゃぐちゃに配置こそされているが辺同士の交差が一切ない状況にある場合、FR algorithmやL-BFGS methodによる最適化は比較的速やかに進んでいく。しかし、そうでない場合、相互の絡み合いによって勾配が小さくなってしまい、通常の連続最適化では最適化が停滞してしまい、その解消は難しい。
