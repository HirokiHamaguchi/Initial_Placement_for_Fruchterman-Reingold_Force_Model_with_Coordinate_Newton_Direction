...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[overlay, remember picture]
        \draw[-{Stealth[length=2mm,width=3mm]}, line width=1mm] (-0.3,1.5) -- (+0.3,1.5);
    \end{tikzpicture}
    \caption{
...
\begin{algorithm}[ht]
    ...
    $\text{Define parameters } \red{k, t, \Delta_t, N_\mathrm{iter}}$\;
    Define initial placement of $v_i \in V$ randomly\;
    \For{\red{$n_\mathrm{iter} \gets 0$ \KwTo $N_\mathrm{iter}$}}{
...
Secondly, we conducted a detailed experiment to investigate the behavior of the proposed algorithm in detail. The result is shown in Fig.~\ref{fig:detail}.

The experiment details are as follows.
We used the 4 algorithms: FR algorithm (\textsf{FR}), the proposed initialization plus FR algorithm (\textsf{SN-FR}), L-BFGS method (\textsf{L-BFGS}), and the proposed initialization plus L-BFGS method (\textsf{SN-L-BFGS}). \textsf{SN} represents the Subspace Newton, which is the proposed algorithm.
We set the maximum number of iterations $N_\mathrm{iter}$ to 200.

We tested with 7 graphs: \texttt{cycle300}, \texttt{jagmesh1}, \texttt{dwt\_1005}, \texttt{btree9}, \texttt{1138\_bus}, \texttt{dwt\_2680}, and \texttt{3elt}. \texttt{cycle300} is a cycle graph with 300 vertices, and btree9 is a perfect binary tree with $2^{9+1}-1=1023$ vertices. Other graphs are from Sparse Matrix Collection~\cite{davis2011university}, and these choice is based on the experiments conducted in Ref.~\cite{8419285}. Thus, although all the graphs are quite sparse so that $\abs{E} / (\abs{V}(V-1)/2)$ is less than 1\%, this is not an arbitrary choice. The important graphs often have such a sparsity.

We first explain the meaning of the figure.
The left plot illustrates the objective function values on the vertical axis versus execution time on the horizontal axis, showcasing the outcomes across 10 trials.
Faint crosses represent the results obtained without initial configuration, while faint circles denote those with initial configuration, plotted every 10 iterations for each trial.
The solid lines correspond to the average of these values across the 10 trials for each approach.
If a trial terminated before reaching 200 iterations, the data was plotted up to the minimum trial count achieved across all trials.
Each trial was conducted with a different seed, meaning that even optimization algorithms that do not incorporate randomness in their process, such as \textsf{FR} or \textsf{L-BFGS}, converged to different local optima due to variations in the initial configuration.
The graph on the right side illustrates the configuration at the 150th iteration (or the final iteration if it concluded before then) for each approach, specifically for the case where the seed is set to zero.

The observations derived from the figure are as follows.
First, the solid plot containing \textsf{SN}, which incorporates the proposed initial configuration method, generally demonstrates superior performance compared to the approach without it, validating the efficacy of the proposed method. A notable exception arises with the \textsf{FR} algorithm, which exhibits pronounced oscillations in the solution, likely due to excessive step sizes leading to overshooting. Although we refrained from altering widely recognized algorithms for fairness, adjusting the step size could enable the proposed method to achieve its intended performance. In fact, even under these conditions, the initial value of $f(X)$—the starting point of the experimental plot—is clearly lower than the objective function values produced by traditional methods.

Additionally, the visualization results support the effectiveness of the proposed initial configuration. In most cases, configurations obtained with \textsf{SN} that include the proposed method better represent the intended geometric arrangement compared to those generated by other methods.

As an overarching trend, regardless of the presence or absence of the proposed initial configuration, the approach utilizing \textsf{L-BFGS} consistently outperforms that employing \textsf{FR}. This finding is consistent with prior research~\cite{6183577}, though, regrettably, this technique remains relatively underutilized. One of the objectives of this paper is to further underscore the efficacy of mathematical optimization approaches, such as \textsf{L-BFGS}, and these results substantiate this assertion.
...
\begin{figure*}[btp]
    ...
    \begin{tabular}{ccccc}
        \multicolumn{5}{c}{\textbf{\texttt{cycle300} ($\abs{V}=300, \abs{E}=300, \text{sparsity}=0.669$\%)}}     \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{jagmesh1} ($\abs{V}=936, \abs{E}=2664, \text{sparsity}=0.609$\%)}}    \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{dwt\_1005} ($\abs{V}=1005, \abs{E}=3808, \text{sparsity}=0.755$\%)}}  \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{btree9} ($\abs{V}=1023, \abs{E}=1022, \text{sparsity}=0.196$\%)}}     \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{1138\_bus} ($\abs{V}=1138, \abs{E}=1458, \text{sparsity}=0.225$\%)}}  \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{dwt\_2680} ($\abs{V}=2680, \abs{E}=11173, \text{sparsity}=0.311$\%)}} \\
        ...
        \multicolumn{5}{c}{\textbf{\texttt{3elt} ($\abs{V}=4720, \abs{E}=13722, \text{sparsity}=0.123$\%)}}      \\
