  ...
  \begin{frame}{Graph Drawing by NetworkX}
    \begin{columns}
      \begin{column}{0.62\columnwidth}
        \large{%
          NetworkX \raisebox{-0.2\baselineskip}{\includegraphics[width=1.5em]{graphDrawing/networkxLogo.png}}~\cite{hagberg2008exploring} is a popular Python library.\\
...
\ifShowHidden
  \begin{frame}{Simplify the Problem (1/2)}
    ...
    Graphs have sparsity $\tccA{\abs{E}} \ll \tccD{\abs{V}^2}$.
    We first separate $f(X)$ into $\tccA{E^\mathrm{a}_{i,j}}$ terms and $\tccD{E^\mathrm{r}}$ terms.
    \begin{mini*}
      {X \in \bbR^{2 \times n}}
...
\ifShowHidden
  \begin{frame}{Simplify the Problem (2/2)}
    ...
    $\tccD{-k^2\log{\norm{x_i-x_j}}} \to \tccD{\norm{x_i - x_j} \geq \epsilon}$ does not lose the problem essence.\\
    Fix the point placement to $Q$ (any two points apart $\epsilon$) $\to$ \textbf{Skip the check of constraints}.
    \begin{mini*}
      {X \in \bbR^{2 \times n}}
      ...
      \addConstraint{x_i}{\neq x_j \quad}{\text{for $1 \leq i < j \leq n$}}
    \end{mini*}
    Only treat \tccA{attractive forces}. Thus, the points set \tccD{$Q$} should be as dense as possible.\\
  ...
  \begin{frame}{Rationale of the Proposed Method}
    \begin{columns}
      \begin{column}{0.5\columnwidth}
        ...
        $\to$ at least $\epsilon$ movement\\
        $\to$ more efficient than direct application.
      \end{column}
      \begin{column}{0.5\columnwidth}
        \begin{figure}[h]
          \centering
...
\ifShowHidden
  \begin{frame}{``objective functions arising from graphs''~\cite{recht-wright}}
    \begin{equation*}
      f(X) = \sum_{\{i,j\}\in E} f_{i,j}(x_i, x_j)
      +\lambda \sum_{i=1}^{n} \Omega_i(x_i)
    \end{equation*}
    General optimization problem arising from graphs.\\
    \textbf{stochastic coordinate descent}~\cite{recht-wright} is a popular method.\\
    ...
    \begin{columns}
      \begin{column}{0.5\columnwidth}
        ...
        Continuous relaxation $\to$ optimization on Riemannian manifolds~\cite{klusContinuousOptimizationMethods2023,klusContinuousOptimizationMethods2023}\\
