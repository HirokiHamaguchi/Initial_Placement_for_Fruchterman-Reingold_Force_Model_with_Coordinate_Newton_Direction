...
\begin{frame}{Graph Drawing by NetworkX}
  \begin{columns}
    \begin{column}{0.62\columnwidth}
      \large{%
        NetworkX \raisebox{-0.2\baselineskip}{\includegraphics[width=1.5em]{graphDrawing/networkxLogo.png}} is a popular Python library.\\
...
\ifShowHidden
  \begin{frame}{``Twist'' Causes Stagnation}
    \large{%
      ...
      $\order{\abs{V}^2}$ per iteration. Slow for large-scale graphs.
    }
    \begin{figure}[htbp]
      \centering
...
\ifShowHidden
  \begin{frame}{Coordinate Newton's Direction}
    $f\colon \bbR^n \to \bbR$ : strictly convex function /  The second order approximation at $x_0$ is
    \begin{equation*}
    ...
    (\textbf{too expensive} ... )\\
    coordinate Newton direction: $d_i = -\nabla^2 f_i(x_i)^{-1} \nabla f_i(x_i)$ \quad \red{Computable!}
  \end{frame}
\fi
