...
\ifShowHidden
  \begin{frame}{Simplify the Problem (2/2)}
    Following previous research, we fix the possible positions $x_i$ to a discrete set of points $Q$:
    \begin{mini*}
      {X \in \bbR^{2 \times n}}
      {\tccA{\sum_{\{i,j\}\in E} \frac{w_{i,j}\norm{x_i - x_j}^3}{3k}} - \tccD{\sum_{i<j} k^2\log{\norm{x_i - x_j}}},}
      {}
...
\ifShowHidden
  \begin{frame}{Experiments Result (gif)}
    \begin{figure}[htbp]
      \centering
      \animategraphics[autoplay,loop,width=0.4\columnwidth]{5}{jagmesh1/CN-L-BFGS/}{0}{0}
    \end{figure}
  \end{frame}
\fi
