    ...
    \begin{figure}[htbp]
      ...
      \begin{minipage}{0.49\hsize}
        ...
        \begin{tikzpicture}
          ...
          \draw[{Stealth[length=4,width=3]}-{Stealth[length=4,width=3]}] ($(M1)+(90:2*\r*\yAdjust)$) -- ($(M2)+(90:2*\r*\yAdjust)$)
          node[midway,circle,fill=white,inner sep=0] {$d = k/a_{i,j}^{1/3}$};

          \draw[-{Stealth[length=2mm]},thick,line cap=round] (+\r,0) -- (+\u,0) node[right] {$F^a_{i,j}(d)$};
  ...
  \begin{frame}{Hardness of graph drawing}
    ...
    \begin{figure}[htbp]
      \begin{minipage}{0.59\hsize}
        \centering
        \animategraphics[autoplay,loop,width=0.8\columnwidth]{5}{imgs/circle/circle-}{1}{1}
  ...
  \begin{frame}{Key Observation: stochastic strategy}
    ...
    \begin{figure}[htbp]
      ...
      \begin{tikzpicture}
        ...
        \draw[-{Stealth[length=10,width=0.4cm]},line width=0.15cm] (-3.1, 0) -- (-1.9, 0);
        \draw[-{Stealth[length=10,width=0.4cm]},line width=0.15cm] (+3.1, 0) -- (+1.9, 0);
      \end{tikzpicture}
    \end{figure}
  \end{frame}
  ...
  \begin{frame}{End of the Presentation}
    \begin{block}{Summary}
      \begin{itemize}
        ...
        \item Hexagonal close-packed structure + RSN result in a powerful initialization
        \item initialization + L-BFGS give a great solution
      \end{itemize}
    \end{block}
    \begin{block}{Acknowledgement}
      \large{
        This work was heavily inspired by the insightful discussions and valuable contributions of \textbf{Pierre-Louis Poirion}, \textbf{Andi Han} and \textbf{Naoki Marumo}.\\
