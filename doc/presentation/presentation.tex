...
\ifShowHidden
    \begin{frame}{\cEText{余談: 試した手法群}}
        \begin{itemize}
            \item シンプルなrandom subspace algorithm
                  \begin{itemize}
                      \item このpair-wise separable functionに対してなら、絶対にrandom subspace的アルゴリズムが上手く機能するはずという直感が、全ての出発点
                      \item \textbf{ダイレクトな適応だと上手くいかないが、適切な工夫でどうにか出来ないか?} というのがこの研究の核心 想定以上にrandom subspaceは有効そうという結論です
                  \end{itemize}
            \item (丸茂先生から頂いた非常に本質的なアドバイスである) 頂点毎にL-BFGSを適用
                  \begin{itemize}
                      \item シンプルなrandom subspace algorithmと同じ理由で遅い
                      \item (cubic regularized Newton以前の根本的な問題)
                  \end{itemize}
            \item CG法
                  \begin{itemize}
                      \item 元々の研究課題の一環として、SciPyが実装しているリスタート機能付きの$\beta^{\text{PR}}_+$と強Wolfe条件によるCG法を調べていました
                      \item Zoutendijk条件による証明について、簡単な部分は理解したつもりですが、subspace化した時にどうすればいいのかは全く分からず
                  \end{itemize}
            \item stochastic CG法 {\small ``Stochastic Conjugate Gradient Algorithm with Variance Reduction''}
                  \begin{itemize}
...
\ifShowHidden
    \begin{frame}{Key Observation: stochastic strategy}
        ...
        \begin{figure}[htbp]
            ...
            \end{tikzpicture}
        \end{figure}
        \cEText{\small{(本当はSGDでなく、Random subspaceと読み替えてもらう方が正しい)}}\\
        \cEText{\small{(FRアルゴリズムはL-BFGSに近いが、前者が苦手なことは「ねじれ」の問題として知られている)}}
    \end{frame}
\fi
...
\ifShowHidden
    \begin{frame}{Key Technique 1: Separation of the objective function}
        ...
        \begin{itemize}
            ...
            \item \cEText{(これはあくまで前座で、Key Technique 2が上手く動作する為の準備)}
        \end{itemize}
    \end{frame}
\fi
    ...
    \begin{frame}{Key Technique 2: Randomized subspace algorithm}
        \begin{itemize}
            \item Researches on Randomized Subspace Newton (\textbf{RSN}) are conducted.
                  ...
                  \end{itemize}
            \item \cEText{(疑問:random subspaceを名乗るのにJL-lemmaは必須か? 提案論文は不使用)}
            \item \cEText{(単なる勘ですが、この状況まで落とし込めば部分空間法が強いはず)}
        \end{itemize}
        \begin{columns}[T]
    ...
    \begin{frame}{Summary}
        \begin{block}{Future Work}
            \begin{itemize}
                ...
                \item \cEText{(少なくとも自分が知る限り and 思う限り)かなり面白い話だと思いますし、多少はやる価値がある内容だと自負しています}
            \end{itemize}
        \end{block}
    \end{frame}
\fi
