  ...
  \begin{frame}{NetworkXによるグラフ描画}
    \begin{columns}
      \begin{column}{0.62\columnwidth}
        \large{%
          \textbf{Fruchterman--Reingold (FR)} 力学モデルが著名\\
          一般的・直感的・シンプルなグラフ描画を実現\\[0.8em]
          \textbf{NetworkX} \raisebox{-0.2\baselineskip}{\includegraphics[width=1.5em]{graphDrawing/networkxLogo.png}}~\cite{hagberg2008exploring} は著名なPythonライブラリ\\
...
\ifShowHidden
  \begin{frame}{``捻じれ'' がもたらす停滞}
    \large{%
      \textbf{捻じれ}とは、不必要な辺の交差や歪んだグラフ構造を指す~\cite{veldhuizenDynamicMultilevelGraph2007,cheongSnapshotVisualizationComplex2018}\\
      $\to$ 特に大規模なグラフ描画において、深刻な問題となる\\
...
\ifShowHidden
  \begin{frame}{Fruchterman--Reingold力学モデル (従来的解釈)}
    \textbf{FR力学モデル}は
    \tccA{\Large{\textbf{a}}}\tccA{ttractive force (引力)}と
    \tccD{\Large{\textbf{r}}}\tccD{epulsive force (斥力)}を用いる~\cite{fruchtermanGraphDrawingForcedirected1991}
    \begin{equation*}
    ...
    \textbf{Multilevel Approach}という複雑だが強力な手法も併用可能 (GraphVizなど)
    \begin{columns}
...
\ifShowHidden
  \begin{frame}{問題の定式化}
    \begin{columns}
      ...
      \end{column}
    \end{columns}
    これらのスカラーポテンシャル (エネルギー) は次のように定義される~\cite{6183577}
    \begin{gather*}
...
\ifShowHidden
  \begin{frame}{貢献1: L-BFGS法による最適化 (NetworkXに実装を提案)}
    \begin{columns}
      \begin{column}{0.3\columnwidth}
        \textbf{L-BFGS法}を用いる\\
        無制約問題に対する微分情報を用いた最適化手法\\[0.5em]

        効率的と知られていた~\cite{6183577}\\[0.5em]

        \textbf{主に実装面での貢献}\\
        軽量な実装が特徴\\
        (NetworkXというOSSで\\次バージョンより採用予定)\\[0.5em]

        次ページで述べる非有界な場合に対する\textbf{モデル修正}\\などを新規に提案
      \end{column}%
      \begin{column}{0.7\columnwidth}
        \begin{figure}[h]
          \centering
...
\ifShowHidden
  \begin{frame}{貢献2: 座標ニュートン方向を用いた初期配置}
    ...
    \textbf{焼き鈍し法}~\cite{ghassemitoosiSimulatedAnnealingPreProcessing2016}を用いた既存研究よりも、より広く適用出来る方法を提案
    \begin{figure}[h]
      \centering
...
\ifShowHidden
  \begin{frame}{貢献2: 問題の簡略化 (2/2)}
    先行研究~\cite{ghassemitoosiSimulatedAnnealingPreProcessing2016}に従い、$x_i$の位置を離散点集合$Q$に固定する:
    \begin{mini*}
      {X \in \bbR^{2 \times n}}
      {\tccA{\sum_{\{i,j\}\in E} \frac{w_{i,j}\norm{x_i - x_j}^3}{3k}} - \tccD{\sum_{i<j} k^2\log{\norm{x_i - x_j}}},}
      {}
...
\begin{frame}{貢献2: 提案アルゴリズム -- 更新則}
  ...
  経験的に、ランダムノイズを加えることが効果的である(SAと同様の原理)
  \begin{equation*}
...
\ifShowHidden
  \begin{frame}{実験結果 1/2}
    \begin{figure}[h]
      ...
      \end{tabular}
      \caption{``BEST''は提案手法の500イテレーション目の状態}
    \end{figure}
  \end{frame}
\fi
...
\ifShowHidden
  \begin{frame}{最後に}
    \begin{block}{まとめ}
      \begin{itemize}
        \item Fruchterman--Reingold レイアウトへの数理最適化によるアプローチを提案
        \item 貢献1: L-BFGS法を用いた描画法のOSS実装
        \item 貢献2: 座標ニュートン方向を用いた効率的な初期配置の計算方法の提案
      \end{itemize}
    \end{block}
    \begin{block}{今後の展望}
      \begin{itemize}
        \item グラフ同型判定問題は、2つのグラフの直和を対称に描画するのと似た問題~\cite{eades1984heuristic,klusContinuousOptimizationMethods2023}\\
              提案手法と似た考えを応用出来ないか?
        \item GraphのSparsity自体をもっと効率的にL-BFGS法で使えないか?
        \item より広く、L-BFGS法自体を更に改善させることは出来ないか? (現在の我々の研究)
      \end{itemize}
    \end{block}
  \end{frame}
\fi
