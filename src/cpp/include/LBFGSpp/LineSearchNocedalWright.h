...
class LineSearchNocedalWright {
 private:
  ...
  static Scalar quad_interp(const Scalar& step_lo, const Scalar& step_hi,
                            const Scalar& fx_lo, const Scalar& fx_hi,
                            const Scalar& dg_lo) {
    using std::abs;
    ...
    // This includes (a) NaN values; (b) too close to the end points; (c) outside the
    // interval In such cases, a bisection search is used
    const bool candid_nan = !(std::isfinite(step_candid));
    const Scalar end_dist =
        std::min(abs(step_candid - step_lo), abs(step_candid - step_hi));
    const bool near_end = end_dist < Scalar(0.01) * abs(sdiff);
    const bool bisect = candid_nan || (step_candid <= std::min(step_lo, step_hi)) ||
                        (step_candid >= std::max(step_lo, step_hi)) || near_end;
 ...
 public:
  ...
  static void LineSearch(Foo& f, const LBFGSParam<Scalar>& param, const Vector& xp,
                         const Vector& drt, [[maybe_unused]] const Scalar& step_max,
    ...
    // We also need to save x and grad for step=step_lo, since we want to return the
    // best step size along the path when strong Wolfe condition is not met
    ...
    for (;;) {
      ...
      if (std::abs(dg) <= test_curv) return;  // Case (4)
      ...
      if (iter >= param.max_linesearch) {
        // throw std::runtime_error("the line search routine reached the maximum number
        // of iterations");
    ...
    for (;;) {
      ...
      if (fx - fx_init > step * test_decr || fx >= fx_lo) {
        ...
        // If reaching here, then the current step satisfies sufficient decrease
        // condition
        step_lo = step;
        fx_lo = fx;
        dg_lo = dg;
      ...
      if (iter >= param.max_linesearch) {
        ...
        if (step_lo <= Scalar(0))
          throw std::runtime_error(
              "the line search routine failed, unable to sufficiently decrease the "
              "function value");
